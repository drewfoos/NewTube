...
export const POST = async (request: Request) => {
    ...
    switch (payload.type as WebhookEvent["type"]) {
        ...
        case "video.asset.deleted": {
            ...
            try {
                // First, find the video to get the Uploadthing keys
                ...
                if (videoToDelete) {
                    ...
                    // Only delete the database record after cleaning up assets
                    await db
                        .delete(videos)
                        .where(eq(videos.id, videoToDelete.id));
                } else {
                    // If no video found, still try to delete any record with that upload ID
                    await db
                        .delete(videos)
                        .where(eq(videos.muxUploadId, data.upload_id));
                }
        ...
        case "video.asset.track.ready": {
            ...
            // Typescript incorrectly says asset_id doesn't exist
